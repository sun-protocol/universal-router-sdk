...
contract SDKExactOutV4Test is CLSunSwapV4Test {
    ...
    function test_sdk_inputReferralUsesRouterDebtSettlement() public {
        ...
        args[0] = "node"; args[1] = vm.envString("SDK_ENCODER");
        args[2] = string.concat(vm.toString(address(token0)), ",", vm.toString(address(token1)));
        args[3] = vm.toString(poolKey0.parameters); args[4] = vm.toString(alice);
        args[5] = vm.toString(uint256(2 ether)); args[6] = "v4";
        args[7] = vm.toString(uint256(1 ether)); args[8] = "100"; args[9] = "0";
        ...
        assertEq(token1.balanceOf(alice), 1 ether);
        assertGt(token0.balanceOf(alice), 0);
        assertEq(token0.balanceOf(address(referral)), ((uint256(1 ether) - 1) * 10000 / 9900 + 1) / 100);
        assertEq(token0.balanceOf(address(router)), 0);
        assertEq(token0.balanceOf(address(safeVault)), 0);
    }
    ...
    function test_sdk_exhaustedLiquidityCannotPassSweep() public {
        ...
        args[2] = string.concat(vm.toString(address(token0)), ",", vm.toString(address(token1)));
        args[3] = vm.toString(poolKey0.parameters); args[4] = vm.toString(alice);
        args[5] = vm.toString(uint256(2_000_000 ether)); args[6] = "v4";
        args[7] = vm.toString(uint256(1_000_000 ether)); args[8] = "0"; args[9] = "0";
        args[10] = args[7];
        (bytes memory commands, bytes[] memory inputs,) = abi.decode(vm.ffi(args), (bytes, bytes[], uint256));
